        ...
        for key, channel_id in user_configs.items():
            try:
                ...
                if member and channel:
                    embed, file = await create_report_data(member, f"📑 定期活動レポート: {member.display_name}")
...
async def on_presence_update(before, after):
    ...
    if prev:
        ...
        if prev['status'] in ["online", "idle", "dnd"] and dur >= 60:
            ...
            try:
                event = {'summary': f"[{after.id}] {st_name}", 'start': {'dateTime': prev['time'].isoformat(), 'timeZone': 'Asia/Tokyo'}, 'end': {'dateTime': now.isoformat(), 'timeZone': 'Asia/Tokyo'}, 'colorId': cid}
    ...
    for guild in bot.guilds:
        ...
        if channel:
            await channel.send(f"🔔 **{after.display_name}** は **{st_display.get(str(after.status), '⚪ オフライン')}** になりました。")

@bot.tree.command(name="register", description="通知先チャンネルを登録")
async def register(interaction: discord.Interaction, user: discord.Member, channel: discord.TextChannel):
    ...
    try:
        ...
        await interaction.followup.send(f"✅ {user.display_name} のレポート送信先を設定しました。")
    except Exception as e: await interaction.followup.send(f"❌ エラー: {e}")

@bot.tree.command(name="status", description="現在のステータスと経過時間を表示")
async def status(interaction: discord.Interaction, member: discord.Member = None):
    ...
    embed = discord.Embed(title=f"現在の状況: {target.display_name}", color=c_map.get(str(target.status), 0x95A5A6))
    embed.add_field(name="ステータス", value=l_map.get(str(target.status), "⚪ オフライン"), inline=True)
    ...
    if target.id in user_status_start:
        ...
        embed.add_field(name="開始時刻", value="不明（再起動後など）", inline=True)
        
    await interaction.response.send_message(embed=embed)

@bot.tree.command(name="report", description="即座に活動レポートを作成")
async def report(interaction: discord.Interaction, member: discord.Member = None):
    await interaction.response.defer()
    target = member or interaction.user
    try:
        embed, file = await create_report_data(target, f"📑 個別活動レポート: {target.display_name}")
        await interaction.followup.send(embed=embed, file=file)
    except:
        await interaction.followup.send("レポート作成中にエラーが発生しました。")
